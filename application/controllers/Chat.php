...
class Chat extends CI_Controller {
    ...
    public function index($id = null) {
        ...
        if (isset($user_id) && !isset($id)) {
            $data['users'] = $this->user->getUsersChat($user_id);
            $data['messages'] = $this->user->getMessages2($user_id);
    ...
    public function chat_with_person($id) {
        $data = array();
        $data['email'] = "";
        $data['id'] = $id;
        $user_id = $this->session->userdata('user_id');
        $data['users'] = $this->user->getUsersChat($user_id);
        $data['messages'] = $this->user->getMessages($user_id, $id);
        $this->load->view('_partials/head', $data);
        $this->load->view('_partials/header', $data);
        $this->load->view('chat/chat_view', $data);
        $this->load->view('_partials/footer');
    ...
    public function process($id) {
        ...
        $post_by_user_id = $this->session->userdata('user_id');
        ...
        $data['users'] = $this->user->insertChat($form_data);
        redirect('/chat/chat_with_person/' . $id);
    }
    ...
    function fetch_provincias() {

        if ($this->input->post('region_id')) {
            echo $this->common->fetch_provincias($this->input->post('region_id'));
        }
    }
    ...
    function fetch_comunas() {
        ...
        if ($this->input->post('provincia_id')) {
            echo $this->common->fetch_comunas($this->input->post('provincia_id'));
        }
    }
    ...
    public function form($product_id = null) {
        ...
        $data['service_name'] = "";
        $data['email'] = "";
        $data['services'] = $this->common->getServices(1);
    ...
    public function upload($id = null) {
        ...
        if ($this->upload->do_upload()) {
            ...
            $imgConfig['image_library'] = 'GD2';
            $imgConfig['source_image'] = './uploads/' . $data['file_dir'];
            $imgConfig['wm_overlay_path'] = './uploads/overlay_pb.png';
            $imgConfig['new_image'] = './uploads/wm_img_' . $data['file_dir'];
            ...
            $data = array();
            $this->load->model('imagen_model', 'image');
